...
### ESP32 Automatic Attendance (Now Integrated!)
- **MAC Address Detection**: Automatically detect student devices when they connect to ESP32 WiFi
- **Firebase Integration**: Direct integration with Firebase database (replaces IFTTT and Google Sheets)
- **Automatic Attendance Marking**: Mark attendance automatically based on device MAC addresses
- **Device Registration**: Students register their device MAC addresses in the app (visible only to administrators)
...
```json
{
  ...
  "macAddress": "AA:BB:CC:DD:EE:FF",  // For ESP32 integration (optional)
  "isPresent": false,
  "createdAt": "timestamp"
}
```
...
```json
{
  ...
  "isPresent": true,
  "markedAt": "timestamp",
  "markedBy": "manual|ESP32",  // Method used to mark attendance
  "macAddress": "AA:BB:CC:DD:EE:FF"  // Only for ESP32-marked attendance
}
```
